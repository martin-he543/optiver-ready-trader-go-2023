...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.legal_move = False; self.crash_imminent = False
        self.previous_order_id = 0
        self.pending_order_id = 0
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        # print("FILLED", self.previous_order_id)
        ...
        if instrument == Instrument.FUTURE:                    
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.etf_position < POSITION_LIMIT and self.legal_move == True:                          # Are BUYING, position grows
                self.bid_id = next(self.order_ids)
                self.bid_price = new_bid_price
                self.send_insert_order(self.bid_id, Side.BUY, new_bid_price, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
                self.bids.add(self.bid_id)

            if self.ask_id == 0 and new_ask_price != 0 and self.etf_position > -POSITION_LIMIT and self.legal_move == True:
                self.ask_id = next(self.order_ids)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if self.pending_order_id > self.previous_order_id:
            self.crash_imminent == True
            # print("CANCELLING THIS ORDER:", self.previous_order_id)
            # self.bids.discard(client_order_id)
            self.send_cancel_order(self.pending_order_id)
            
        print("ORDER FILLED MESSAGE", self.previous_order_id, [client_order_id, self.pending_order_id], self.crash_imminent)
    ...
    def on_order_status_message(self, client_order_id: int, fill_volume: int, remaining_volume: int, fees: int) -> None:
        ...
        print("ORDER STATUS MESSAGE", self.previous_order_id, client_order_id)
        self.pending_order_id = client_order_id
        
        # if self.pending_order_id > self.previous_order_id:
        #     print("CANCELLING THIS ORDER:", self.previous_order_id)
        #     # self.bids.discard(client_order_id)
        #     self.send_cancel_order(self.previous_order_id)
        #     self.logger.info("received cancellation order status for order %d with fill volume %d remaining %d and fees %d",
        #         client_order_id, fill_volume, remaining_volume, fees)
